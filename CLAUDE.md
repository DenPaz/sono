...
- Reusable template components live under `templates/components/` and are rendered with
  ...
  (e.g. `<c-forms.input :field="form.email" />`). Cotton's component dir is configured via `COTTON_DIR = "components"`.
...
- The front end uses Tailwind CSS 4 and DaisyUI 5. There is no `tailwind.config.js` — all
  Tailwind / DaisyUI configuration lives in CSS files under `static/css/` (see
  `static/css/base/daisyui.css` for theme configuration). The project ships 6 custom DaisyUI
  themes: `light` (default), `dark`, `contrast`, `material`, `dim`, `material-dark`.
...
- The main database is Postgres (a SQLite fallback is configured for convenience but not used in practice).
- Celery is used for background jobs and scheduled tasks (`django-celery-beat` for the scheduler).
  In `local` settings Celery runs eagerly (`CELERY_TASK_ALWAYS_EAGER = True`).
...
```
apps/
  core/      # BaseModel, validators, middleware, template tags, form mixins
...
templates/   # Django templates (including templates/components/ for django-cotton)
locale/      # gettext .po files
fixtures/
```
...
Tests use `pytest` (with `pytest-django`) and `factory-boy`. The test settings module
(`config.settings.test`) is wired up via `pyproject.toml`, and the database is reused by
default (`--reuse-db`).
...
`compilemessages`, ignoring `node_modules`, `venv`, and `.venv`.
...
A `pre-commit` config covers most of these; install hooks with the `pre-commit-install`
target (already part of `make setup`).
...
- Use standard Django template syntax. The project also uses `django-cotton` — components from `templates/components/` are referenced as `<c-namespace.name attr="value" />`. Pass Python expressions as attributes with the `:attr="..."` syntax.
...
- When using HTMX, follow progressive enhancement patterns. The project ships an
  `HtmxMessagesMiddleware` that surfaces Django messages on HTMX responses via the
  `messages` event — listen for it on the client (see `static/js/partials/toasts.js`).
- Use Alpine.js for client-side interactivity that doesn't require server interaction.
  Register data components on the `alpine:init` event (see `topbar.js`).
