...
## Project-specific notes (Sono)

- daisyUI is already installed and configured in `static/css/base/daisyui.css`. Do not modify the plugin config or add new themes without updating that file.
- The project uses 6 custom themes: `light` (default), `dark`, `contrast`, `material`, `dim`, `material-dark`. Dark themes are `dark`, `dim`, and `material-dark`.
- Active theme is set via `data-theme` on `<html>`. It is managed by `LayoutCustomizer` in `static/js/partials/layout.js` and persisted in `localStorage` under `__SONO_CONFIG__`.
- There is no need to use `dark:` variants with daisyUI color names — colors update automatically with the theme.
- To override daisyUI styles, append `!` to the Tailwind utility class (e.g. `bg-red-500!`). Do not use `!important`.
- Do not use `tailwind.config.js` — it is deprecated in Tailwind CSS v4. All config lives in CSS files.
- Custom layout CSS variables (`--layout-sidebar-background`, `--layout-topbar-background`, `--root-bg`) are defined per theme in `static/css/base/daisyui.css` and used in `static/css/pages/layout.css`.
- Icon classes follow the pattern `iconify {prefix}--{icon-name}` (e.g. `iconify lucide--settings`). Available prefixes: `lucide`, `hugeicons`, `ri`.
...
- To add an avatar, use `<div class="avatar chat-image">` and nest the avatar content inside
...
```html
<ul class="menu w-80 min-h-full p-4 text-base-content bg-base-100">
  <li><a>Item 1</a></li>
  <li><a>Item 2</a></li>
</ul>
```
...
<div class="drawer lg:drawer-open">
  ...
  <div class="drawer-side">
    ...
    ></label>
    <ul class="menu w-80 min-h-full p-4 bg-base-200">
...
<div class="fab">
  ...
  <div class="fab-close">
    Close <span class="btn btn-lg btn-error btn-circle">✕</span>
  </div>
  <div>{Label1}<button class="btn btn-lg btn-circle">{Icon1}</button></div>
  <div>{Label2}<button class="btn btn-lg btn-circle">{Icon2}</button></div>
  <div>{Label3}<button class="btn btn-lg btn-circle">{Icon3}</button></div>
...
<div class="fab">
  ...
  <div class="fab-main-action">
    {LabelMainAction}<button class="btn btn-lg btn-secondary btn-circle">
      {IconMainAction}
    </button>
  </div>
  <div>{Label1}<button class="btn btn-lg btn-circle">{Icon1}</button></div>
...
<div class="fab fab-flower">
  ...
  <button class="btn btn-lg btn-circle fab-main-action">
    {IconMainAction}
  </button>
  <button class="btn btn-lg btn-circle">{Icon1}</button>
  <button class="btn btn-lg btn-circle">{Icon2}</button>
...
```html
<span class="font-title text-7xl text-rotate max-md:text-3xl">
...
<span>
  ...
  <span class="text-rotate">
    <span>
      <span class="px-2 text-teal-800 bg-teal-400">Designers</span>
      <span class="px-2 text-red-800 bg-red-400">Developers</span>
      <span class="px-2 text-blue-800 bg-blue-400">Managers</span>
    </span>
  </span>
</span>
```
...
```html
<span class="font-title text-7xl text-rotate leading-[2] max-md:text-3xl">
  <span class="justify-items-center">
    <span>📐 DESIGN</span>
    <span>⌨️ DEVELOP</span>
    <span>🌎 DEPLOY</span>
